...
<script type="text/javascript">
	...
	function gethomeworktitle(data) {
		$('#homeworktitle').combobox({
			...
			textField : 'title',
			onLoadSuccess : function(data) {
				$("#homeworktitle").combobox("setValue", data[0].id);
				statistics();
			},
		});
	}
	...
	function statistics() {
		...
		var homeworkId = $('#homeworktitle').combobox("getValue");
		echarts(classId, homeworkId)
	}

	function echarts(classId, homeworkId) {
		// 初始化图表标签
		$("#echarts").prop(
				"src",
				"<%=request.getContextPath()%>/view/echartsHomework.jsp?classId=" + classId
...
<body style="margin: 0px 0px 0px 0px; background: #F4F4F4;">
	<div>
		<div style="height: 30px; margin-top: 10px;">
			<span id="color" style="margin-left: 43%; font-size: 20px; color: rgb(26, 51, 243);">作业情况统计</span>
		</div>
